...
class Detect(nn.Module):
    # YOLO Detect head for detection model
...
class Segment(Detect):
    # YOLO Segment head for segmentation model
...
class Panoptic(Detect):
    # YOLO Panoptic head for panoptic segmentation model
...
class ClassificationModel(BaseModel):
    ...
    def _from_detection_model(self, model, nc=1000, cutoff=10):
        ...
        c.i, c.f, c.type = m.i, m.f, 'model.common.Classify'  # index, from, type
...
if __name__ == '__main__':
    ...
    elif opt.test:  # test all model
        for cfg in Path(ROOT / 'model').rglob('yolo*.yaml'):
            try:
                _ = Model(cfg)
